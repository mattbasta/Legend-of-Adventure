	...
	getNeighbors := func(from pathStep) []pathStep {
		...
		for i := -1; i < 2; i++ {
			for j := -1; j < 2; j++ {
				...
				if !hitmap.Fits(float64(x), float64(y), w, h) {
					continue
				}
				// Ignore tiles that we've already enumerated.
				if _, ok := closedCoords[[2]int{x, y}]; ok {
					continue
				}

				// Don't allow the entity to cut corners
				if i != 0 && j != 0 {
					if !hitmap.Fits(float64(from.X+j), float64(from.Y), w, h) {
						continue
					}
					if !hitmap.Fits(float64(from.X), float64(from.Y+i), w, h) {
						continue
					}
				}
