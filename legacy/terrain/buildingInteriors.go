...
func ApplyBuildingInterior(terrain *Terrain, buildingType, parent string) {
	...
	if Chance(rng) {
		setRoom(0, 2, getRoomType())
	}
	if Chance(rng) || !filled(0, 2) {
		setRoom(2, 2, getRoomType())
	}
	if buildingType != REGIONTYPE_SHOP && Chance(rng) {
		setRoom(1, 1, getRoomType())
	}
	if (filled(0, 2) || filled(1, 1)) && Chance(rng) {
		setRoom(0, 1, getRoomType())
	}
	if (filled(2, 2) || filled(1, 1)) && Chance(rng) {
		setRoom(2, 1, getRoomType())
	}
	if filled(0, 1) && Chance(rng) {
		setRoom(0, 0, getRoomType())
	}
	if filled(2, 1) && Chance(rng) {
		setRoom(2, 0, getRoomType())
	}
	if (filled(0, 0) || filled(2, 0)) && Chance(rng) {
		setRoom(2, 0, getRoomType())
	}
...
func drawShopLobby(terrain *Terrain, x, y int, rng *rand.Rand) {
	...
	for i := y + 5; i < y+ROOMSIZE_HEIGHT-2; i++ {
		if terrain.Tiles[i][x+1] != 1 {
			continue
		}
		if terrain.Tiles[i][x] == 1 && terrain.Tiles[i][x+2] == 1 {
			continue
		}
	...
	for i := y + 5; i < y+ROOMSIZE_HEIGHT-2; i++ {
		if terrain.Tiles[i][x+ROOMSIZE_WIDTH-2] != 1 {
			continue
		}
		if terrain.Tiles[i][x+ROOMSIZE_WIDTH-1] == 1 && terrain.Tiles[i][x+ROOMSIZE_WIDTH-3] == 1 {
			continue
		}
		if rng.Intn(10) < SHOP_LOBBY_POT_ODDS {
